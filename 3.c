...
/* waitingRoom Limits the # of customers allowed to enter the waiting room at
one time. */
sem_t waitingRoom;
// barberChair ensures mutually exclusive access to the barber chair.
sem_t barberChair;
// barberPillow is used to allow the barber to sleep until a customer arives.
sem_t barberPillow;
/* seatBelt is used to make the customer to wait until the barber is done
...
void *barber(void *junk)
{
	...
	and can tell if there are customers still on the way to his shop. */
	while (!allDone)
	{
